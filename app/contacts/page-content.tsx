...
function ContactsPageContent() {
  const { contacts, loading, error, refresh, deleteContact, updateContact } = useContacts();
  ...
  const handleBulkDelete = useCallback(async (ids: string[]) => {
    // Optimistic updates happen inside each deleteContact call
    try {
      const results = await Promise.all(ids.map(id => deleteContact(id)));
  ...
  const handleBulkTag = useCallback(async (ids: string[], tagNames: string[]) => {
    // Optimistic updates happen inside each updateContact call
    try {
      const results = await Promise.all(
        ids.map(async (id) => {
          ...
          if (existing) {
            ...
            return updateContact(id, { tags: merged });
          }
          return true;
        })
      );
      ...
      if (failures.length > 0) {
        ...
        toast.success(`${ids.length} contact(s) tagged`);
      }
    } catch {
      refresh();
      toast.error('Failed to tag contacts');
    }
  }, [updateContact, contacts, refresh]);
